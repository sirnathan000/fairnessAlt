...
def altRatio(partition, QID_NAMES, goal, outcome, names):
#here all the column names are set using the names var
    columns = names
...
def choose_dimension(partition, QID_NAMES, Protected_att, goal, outcome, names):
    """
    ...
    for i in range(QI_LEN):
        ...
        if QID_NAMES[i] in Protected_att:
            ratio = altRatio(partition, QID_NAMES[i], goal, outcome, names)
...
def anonymize(partition,QID_NAMES, Protected_att, goal, outcome, names):
    """
...
#        print(record)
    if check_splitable(partition) is False:
        ...
        return
    # Choose dim
    dim = choose_dimension(partition, QID_NAMES, Protected_att, goal, outcome, names)
    if dim == -1:
        print("Error: dim=-1")
        pdb.set_trace()
    ...
    if len(sub_partitions) == 0:
        ...
        anonymize(partition, QID_NAMES, Protected_att, goal, outcome, names)
    else:
        for sub_p in sub_partitions:
            anonymize(sub_p, QID_NAMES, Protected_att, goal, outcome, names)
...
def mondrian(att_trees, data, k, QI_num, SA_num,ATT_NAMES, QID_NAMES, Protected_att, goal, outcome, names):
    """
    ...
    middle = []
    wtemp = []
#    print("this is the print in alt_modified_mondrian")
    ...
    anonymize(whole_partition,QID_NAMES, Protected_att, goal, outcome, names)
